...
layui.define(['element', 'common'], function (exports) {
     ...
     * @param {Object} data
     */
    Tab.prototype.deleteByIndex = function (index) {

        var that = this;
        var _config = that.config;
        index = index || _config.elem.children('ul.layui-tab-title').children('li[lay-id]').eq(0).attr("lay-id");
        element.tabDelete(ELEM.tabFilter, index);
                            ...
                            switch (target) {
                                case 'refresh': //刷新当前
                                    if (id) {
                                        ...
                                        var src = ELEM.contentBox.find('iframe:eq(0)')[0].src;
                                        ELEM.contentBox.find('iframe:eq(0)')[0].src = src;
                                    }
                                    break;
    ...
    Tab.prototype.openTab = function (targetUrl, options) {

        targetUrl = targetUrl.toLocaleLowerCase();
        var that = this;
        var _config = that.config;

        var id = this.getCurrentTabId();

        options = options || {};

        //refersh/reload
        options.refershType = options.refershType || "reload";
        options.autoClose = options.autoRefresh || true;
        ...
        if (targetUrl) {
            ...
            if (targetUrl.indexOf("index/main") > -1) {
                $("li:eq(0)", ELEM.titleBox).click();
                ...
                $("iframe").each(function (index) {
                    var src = this.src.toLocaleLowerCase();
                    var self = this;
                    if (src.indexOf(targetUrl) > -1) {
                        $("li", $("ul.layui-tab-title")).eq(index).click();
                ...
                var $admin_navbar_side = $("#admin-navbar-side", top.window.document);
                var $admin_navbar_side_ul = $("ul", $admin_navbar_side);
                var $admin_navbar_side_ul_li = $("li", $admin_navbar_side_ul);

                $admin_navbar_side_ul_li.each(function (i, item) {

                    $("dd", $("dl", $(this))).each(function () {

                        var self = this;
                        var title = $(this).attr("title");
                        var $a = $("a", $(this));

                        if ($a && $a.attr("data-url")) {

                            var href = $a.attr("data-url").toLocaleLowerCase();
                            if (href.indexOf("/") > -1) {
                                ...
                                if (targetUrl.indexOf(href) > -1) {
                                    ...
                                    if (tabIndex === -1) {
                                        ...
                                        element.tabChange(ELEM.tabFilter, that.getTabId(title));
                                        //自动刷新
                                        if (options.refershType == "reload") {
                                            _config.elem.find('div.layui-tab-content > div.layui-show').children('iframe')[0].contentWindow.location.reload();
                                        }

                                        if (options.refershType == "refersh") {
                                            _config.elem.find('div.layui-tab-content > div.layui-show').children('iframe')[0].src = targetUrl;
                                        }
                                    }
                                    return false;
